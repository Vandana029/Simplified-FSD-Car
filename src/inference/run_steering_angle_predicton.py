...
class DrivingSimulator:
    def __init__(self, predictor, data_dir, steering_image_path, is_windows=False):
        ...
        self.steering_image = cv2.imread(steering_image_path, cv2.IMREAD_GRAYSCALE)

        if self.steering_image is None:
            raise FileNotFoundError(f"Steering image not found: {steering_image_path}")

        self.is_windows = is_windows

        if len(self.steering_image.shape) == 3:
